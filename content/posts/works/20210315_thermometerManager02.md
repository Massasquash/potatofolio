---
title: "[温度管理ツール02][GAS]APIに必要な情報をオブジェクトとクラスにまとめてみる"
date: 2021-03-15
lead: "前回のコードをリファクタリング、センサーデータを保持するクラスを作る"
categories:
  - "Works"
---

# [温度管理ツール02][GAS]API通信に関する部分をまとめて活用しやすくする

#●技術アウトプット

20210314_thermometermanager02
...
次のLINEに送ったり積算温度を算出したりというメインの実装をしていきたいところですが、その前に今後の実装のためにリファクタリングをしてお区ことにしました。  
APIを使うための情報と処理を、オブジェクトやクラスを活用してメインのコードとは分離して、整理しておきたいと思います。

このようなやり方がベストプラクティスなのかわからないのですが、こんな風にできるんだと参考程度に見ていただければ幸いです。

## 1. 整理すること
メインのスクリプトとは別に、以下のようにファイル分けして整理することにしました。

### 認証に必要な秘匿情報 `credentials.gs`
...
### T&D ThermoのAPIに関する情報と処理 `apis.gs`
